...
def is_spamming(user_id):
    ...
    user_requests[user_id] = [timestamp for timestamp in user_requests[user_id] if now - timestamp < timedelta(seconds=60)]

    if len(user_requests[user_id]) > 10:
        ban_until = datetime.now() + timedelta(days=1)
        banned_users[str(user_id)] = {"until": ban_until.strftime("%Y-%m-%d %H:%M:%S")}
        save_banned_users()
        bot.send_message(user_id, f"Вы забанены за спам до {ban_until.strftime('%Y-%m-%d %H:%M:%S')}.")
        return True
